...
.DESCRIPTION
The New-LMDynamicServiceInsight function creates a new LogicMonitor Service template with the specified parameters.
...
.EXAMPLE
New-LMDynamicServiceInsight -Name "Application Services by Site" -Description "Default LM service template for application services"
...
#>
function New-LMDynamicServiceInsight {
    ...
    if ($Script:LMAuth.Valid -and $Script:LMAuth.Type -eq "SessionSync") {
        ...
        Write-Information "[INFO]: Creating Dynamic Service Insight '$Name' with $($Cardinality.Count) cardinality field(s), FilterType=$FilterType, MembershipEvaluationInterval=$MembershipEvaluationInterval, CreateGroup=$CreateGroup."
        ...
        if ($PSCmdlet.ShouldProcess($Message, "Create Service Template")) {
            ...
            $createdId = $Response.data.byId.RestServiceTemplate.PSObject.Properties.Name | Select-Object -First 1
            if ($createdId) {
                Write-Information "[INFO]: Dynamic Service Insight created: id=$createdId, name='$Name'. Disabled by default - use Set-LMDynamicServiceInsight -Id $createdId -IsEnabled `$true to activate it."
            }
            else {
                Write-Warning "Dynamic Service Insight '$Name' request completed but no id was found in the response - verify creation via Get-LMDynamicServiceInsight."
            }
